...
// App passes down who's logged in, and a function to log them out
...
function ChatScreen({ currentUser, onLogout }: ChatScreenProps) {
    ...
    useEffect(() => {
        ...
        // Ask the server for the message history now that the chat is open.
        // (The automatic load on connect may have already happened before
        // this screen existed, so we request it explicitly here.)
    ...
    return (
        <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-purple-600 to-blue-500 text-white px-6 py-8">
            ...
            {/* Header: logo + name on the left, who you are + logout on the right */}
            <div className="w-full max-w-md flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={onLogout}
                        className="bg-white/20 hover:bg-white/30 text-white text-sm font-semibold px-3 py-1 rounded-lg transition"
                    >
                        Log out
                    </button>
                </div>
